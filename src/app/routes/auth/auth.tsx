...
function BrandPanel() {
  const highlights = [
    `Generate ${BRAND.format}-format drafts in minutes`,
  ...
  return (
    <div className="flex flex-col justify-between p-10 h-full">
      <BrandLogo className="[&_span]:text-white [&_span]:text-lg" />

      <div className="space-y-6">
        <div className="space-y-2">
          <h1 className="text-3xl font-semibold text-white leading-snug text-balance">
            The platform built<br />for Filipino teachers.
          </h1>
          <p className="text-sm text-white/70 leading-relaxed">
            Start with {BRAND.format}-format lesson planning. More teacher tools on the way.
          </p>
        </div>
      ...
      <p className="text-xs text-white/40 leading-relaxed">
        {BRAND.name} supports teachers at every stage — draft, review, and publish.
      </p>
    </div>
  )
}
    ...
    <main className="min-h-screen flex flex-col lg:flex-row">
      <div className="lg:w-[46%] lg:min-h-screen lg:sticky lg:top-0 bg-primary flex-shrink-0">
        <div className="flex items-center gap-3 px-5 py-4 lg:hidden">
          <BrandLogo className="[&_span]:text-white [&_span]:text-base" />
        </div>
                ...
                <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
                  ...
                  <TabsContent value="register" className="mt-0">
                    <div className="mb-5">
                      ...
                      <p className="text-sm text-muted-foreground mt-0.5">
                        Join {BRAND.name} and start generating DepEd-compliant lesson plans.
                      </p>
                    </div>
                    <div className="flex flex-col gap-4">
                      <GoogleButton
